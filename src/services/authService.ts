...
export const authService = {
    ...
    async setSession(access_token: string, refresh_token: string) {
        ...
        // Force a session check to update the store and profile
        ...
        try {
            ...
            if (fetchError && fetchError.code === 'PGRST116') {
                ...
                if (insertError) {
                    ...
                    if (retryProfile) {
                        const isAutoGeneratedUsername = !retryProfile.username ||
                            /^user_[a-f0-9]{6}_[a-z0-9]+$/.test(retryProfile.username);
                        const needsSetup = isAutoGeneratedUsername ||
                            !retryProfile.date_of_birth ||
                            !retryProfile.gender;
                        return { profile: retryProfile as UserProfile, needsSetup };
                    }
